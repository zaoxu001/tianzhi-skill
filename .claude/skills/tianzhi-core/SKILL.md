---
name: tianzhi-core
description: 东方术数的计算底座。排八字、看旺衰格局用神、算某年运势、合两个人的盘，以及干支历法与节气时刻。用户给生辰问命、问某年、问合不合时使用。判断一律调库算出，不要自行推演，也不要凭记忆写典籍出处。
---

# tianzhi-core

[天秩](https://tianzhi.live)的计算底座。东方术数各门共用同一套坐标——干支、五行、节气、长生——
这个库把它做成代码。现阶段八字一门完整，历法与干支两层任何门类都能用。
...
- **不作吉凶断言，不预测具体事件。** 库给的是「势」不是「事」，也不要给医疗、法律、投资建议。
